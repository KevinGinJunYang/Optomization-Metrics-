...
def mini_batch_gradient_descent(y, x, theta, max_iters, alpha, metric_type, mini_batch_size):
    ...
    num_of_samples = len(x)
    for i in range(max_iters):
        for j in range(math.floor(num_of_samples/mini_batch_size)):

            rand = np.random.permutation(num_of_samples)
            x = x[rand]
            y = y[rand]

            x_var = x[0:mini_batch_size]
            y_var = y[0:mini_batch_size]

            gradient = -2 * x_var.T.dot(y_var - x_var.dot(theta)) / num_of_samples
            theta = theta - alpha * gradient
            loss = compute_loss(y_var, x_var, theta, metric_type)
    ...
    print("Mini_batch({bi}/{ti}): loss={l}, w={w}, b={b}".format(
        bi = i, ti = max_iters - 1, l = loss, w = theta[0], b = theta[1]))
